...
void Graph::bestFirstSearch(int start, int goal)
{
	...
	priority_queue <Vertex, vector<Vertex>, priComparer> open;
	...
	for (i = 0; i < n; i++)
	{
 	...
 	open.push(Vertex(start, heuristic[start])); 
	while (!open.empty())
	{
		do
		{
		...
		for (i = 1; i <= n; i++)
		{
			...
			{
				open.push(Vertex(i, heuristic[i]));
				parent[i] = v;
			}
		}
	}
